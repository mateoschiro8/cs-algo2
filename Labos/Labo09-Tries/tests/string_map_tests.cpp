...
TEST(string_map_test, pruebaManual) {
    string_map<int> prueba;
    prueba.insert(make_pair("!#$", 5));
    prueba.insert(make_pair("!#$T", 5));

    EXPECT_TRUE(prueba.count("!#$") == 1);

    prueba.erase("!#$");

    EXPECT_FALSE(prueba.count("!#$") == 1);
    EXPECT_TRUE(prueba.count("!#$T") == 1);
}
