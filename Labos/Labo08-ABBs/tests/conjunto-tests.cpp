...
TEST(conjunto_test, test_remover_caso_dos_hijos_raiz) {
    ...
    /* for (int i = 5; i < 9; i++) {
        EXPECT_EQ(c.pertenece(i), i != 5);
    }
    */
}
// Test manual
TEST(conjunto_test, test_remover_caso_dos_hijos_raiz_hijosIzq) {
    Conjunto<int> c;
    c.insertar(8);
    c.insertar(5);
    c.insertar(9);
    ...
    EXPECT_TRUE(c.pertenece(5));
    EXPECT_TRUE(c.pertenece(6));
    EXPECT_TRUE(c.pertenece(9));
    EXPECT_FALSE(c.pertenece(8));
}
